<?PHP
   ...
   $data = fgetcsv($file, 5000000, ";"); //Remove if CSV file does not have column headings
      ...
      while (($line = fgetcsv($file, 5000000, ";")) !== FALSE) {
            ...
            $dragonpay_ws_refNo =  $line[0];
            $dragonpay_ws_refDate = $line[1];
            $dragonpay_ws_merchantId = $line[2];
            $dragonpay_ws_merchantTxnId = $line[3];
            $dragonpay_ws_amount = $line[4];
            $dragonpay_ws_currency = $line[5];
            $dragonpay_ws_description = $line[6];
            $dragonpay_ws_email = $line[7];
            $dragonpay_ws_status = $line[8];
            $dragonpay_ws_procId = $line[9];
            $dragonpay_ws_procMsg = $line[10];
            $dragonpay_ws_billerId = $line[11];
            $dragonpay_ws_settleDate  = $line[12];
           ...
           $orderIncrementId = $dragonpay_ws_merchantTxnId;
           ...
           $order = Mage::getModel('sales/order')->loadByIncrementId($orderIncrementId);
             ...
             // START:     process for each line
                    ...
                    if ($dragonpay_ws_status == 'S'){
                                 ...
                                 ## Check that $dragonpay_ws_refNo has not been previously processed sucessfully in Magento
